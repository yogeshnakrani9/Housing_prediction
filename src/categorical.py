...
class CategoricalFeatures:
    def __init__(self, df, categorical_features, encoding_type, handle_na=False):
        """
        df: pandas dataframe
        categorical_feature: list of column name, e.g. ["ord_1", "nom_0".....]
    ...
    def fit_transform(self):
        if self.enc_type == "label":
            ...
            raise Exception("Encoding type is not understood")

    def transform(self, dataframe):
        """
        use : this will be used on test data
        """
        if self.handle_na:
            for c in self.cat_feats:
                dataframe.loc[:, c] = self.df.loc[:, c].astype(str).fillna("-9999999")
